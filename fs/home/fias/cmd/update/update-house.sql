update fias.house01 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house01 im
         join fias.house01 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house02 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house02 im
         join fias.house02 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house03 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house03 im
         join fias.house03 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house04 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house04 im
         join fias.house04 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house05 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house05 im
         join fias.house05 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house06 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house06 im
         join fias.house06 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house07 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house07 im
         join fias.house07 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house08 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house08 im
         join fias.house08 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house09 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house09 im
         join fias.house09 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house10 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house10 im
         join fias.house10 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house11 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house11 im
         join fias.house11 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house12 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house12 im
         join fias.house12 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house13 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house13 im
         join fias.house13 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house14 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house14 im
         join fias.house14 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house15 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house15 im
         join fias.house15 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house16 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house16 im
         join fias.house16 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house17 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house17 im
         join fias.house17 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house18 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house18 im
         join fias.house18 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house19 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house19 im
         join fias.house19 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house20 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house20 im
         join fias.house20 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house21 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house21 im
         join fias.house21 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house22 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house22 im
         join fias.house22 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house23 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house23 im
         join fias.house23 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house24 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house24 im
         join fias.house24 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house25 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house25 im
         join fias.house25 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house26 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house26 im
         join fias.house26 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house27 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house27 im
         join fias.house27 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house28 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house28 im
         join fias.house28 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house29 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house29 im
         join fias.house29 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house30 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house30 im
         join fias.house30 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house31 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house31 im
         join fias.house31 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house32 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house32 im
         join fias.house32 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house33 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house33 im
         join fias.house33 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house34 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house34 im
         join fias.house34 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house35 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house35 im
         join fias.house35 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house36 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house36 im
         join fias.house36 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house37 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house37 im
         join fias.house37 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house38 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house38 im
         join fias.house38 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house39 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house39 im
         join fias.house39 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house40 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house40 im
         join fias.house40 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house41 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house41 im
         join fias.house41 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house42 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house42 im
         join fias.house42 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house43 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house43 im
         join fias.house43 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house44 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house44 im
         join fias.house44 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house45 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house45 im
         join fias.house45 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house46 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house46 im
         join fias.house46 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house47 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house47 im
         join fias.house47 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house48 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house48 im
         join fias.house48 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house49 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house49 im
         join fias.house49 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house50 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house50 im
         join fias.house50 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house51 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house51 im
         join fias.house51 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house52 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house52 im
         join fias.house52 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house53 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house53 im
         join fias.house53 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house54 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house54 im
         join fias.house54 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house55 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house55 im
         join fias.house55 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house56 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house56 im
         join fias.house56 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house57 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house57 im
         join fias.house57 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house58 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house58 im
         join fias.house58 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house59 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house59 im
         join fias.house59 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house60 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house60 im
         join fias.house60 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house61 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house61 im
         join fias.house61 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house62 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house62 im
         join fias.house62 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house63 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house63 im
         join fias.house63 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house64 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house64 im
         join fias.house64 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house65 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house65 im
         join fias.house65 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house66 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house66 im
         join fias.house66 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house67 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house67 im
         join fias.house67 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house68 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house68 im
         join fias.house68 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house69 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house69 im
         join fias.house69 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house70 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house70 im
         join fias.house70 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house71 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house71 im
         join fias.house71 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house72 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house72 im
         join fias.house72 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house73 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house73 im
         join fias.house73 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house74 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house74 im
         join fias.house74 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house75 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house75 im
         join fias.house75 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house76 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house76 im
         join fias.house76 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house77 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house77 im
         join fias.house77 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house78 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house78 im
         join fias.house78 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house79 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house79 im
         join fias.house79 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house83 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house83 im
         join fias.house83 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house86 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house86 im
         join fias.house86 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house87 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house87 im
         join fias.house87 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house89 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house89 im
         join fias.house89 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house91 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house91 im
         join fias.house91 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house92 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house92 im
         join fias.house92 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
update fias.house99 fa
SET aoguid     = im.aoguid,
    buildnum   = im.buildnum,
    enddate    = im.enddate,
    eststatus  = im.eststatus,
    housenum   = im.housenum,
    statstatus = im.statstatus,
    ifnsfl     = im.ifnsfl,
    ifnsul     = im.ifnsul,
    okato      = im.okato,
    oktmo      = im.oktmo,
    postalcode = im.postalcode,
    startdate  = im.startdate,
    strucnum   = im.strucnum,
    strstatus  = im.strstatus,
    terrifnsfl = im.terrifnsfl,
    terrifnsul = im.terrifnsul,
    updatedate = im.updatedate,
    normdoc    = im.normdoc,
    counter    = im.counter,
    cadnum     = im.cadnum,
    divtype    = im.divtype
from import.house99 im
         join fias.house99 fi
              on im.houseguid = fi.houseguid
                  and im.houseid = fi.houseid
where im.houseguid = fa.houseguid
  and im.houseid = fa.houseid;
